...
def test_k_nearest_at_rank_orders_by_cascade_distance():
    fb = _fitted_calibrated()
    hits = fb._k_nearest_at_rank("species", np.array([0.0, 0.0]), latlon_row=None, k=2)

    assert len(hits) == 2
    assert hits[0].label == "A a"
    assert hits[1].label == "B b"
